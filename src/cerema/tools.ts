...
function humanRequestPath(sessionId: string): string {
 ...
 */
function findChromeWindowPid(sessionManager: SessionManager, managedSessionId: string): string | undefined {
  const session = sessionManager.getSession(managedSessionId);
  if (!session) return undefined;

  const chromeProcess = session.browser.process();
  if (!chromeProcess?.pid) return undefined;

  // Essayer de trouver une fenêtre liée au PID Chrome
  try {
    const output = execSync(
      `xdotool search --onlyvisible --pid ${chromeProcess.pid}`,
    ...
    try {
      const output = execSync(
        'xdotool search --onlyvisible --name "Chrome" --class "chrome"',
        { encoding: 'utf-8', timeout: 5000 },
      );
      const windows = output.trim().split('\n').filter(Boolean);
      return windows[0];
...
async function requestHuman(
  ...
  raison: string,
  timeout_s: number,
): Promise<{ status: 'resumed' | 'timeout'; raison: string; timeout_s: number }> {
...
function removeHumanState(sessionId: string): void {
  try {
 ...
 */
export function registerL5Tools(
  mcpServer: McpServer,
  sessionManager: SessionManager,
  managedSessionId: string,
): void {
  ...
  mcpServer.tool(
    'raise_window',
    'Met la fenêtre de la session appelante au premier plan dans le bureau distant. '
