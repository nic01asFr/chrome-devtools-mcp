...
import { parseChannel, parseUrlPatterns, resolveProfileDir, getForcedFlags, getDefaultViewport, } from './config.js';
// ---------------------------------------------------------------------------
// parseChannel
// ---------------------------------------------------------------------------
describe('parseChannel', () => {
    ...
    it('lève pour un canal invalide', () => {
        expect(() => parseChannel('firefox')).toThrow('CDM_CHANNEL invalide: "firefox"');
        expect(() => parseChannel('chromium')).toThrow('CDM_CHANNEL invalide: "chromium"');
    });
...
describe('parseUrlPatterns', () => {
    ...
    it('parse un CSV multiple', () => {
        const result = parseUrlPatterns('https://example.com, https://api.test.com,https://internal.corp');
...
describe('resolveProfileDir', () => {
    ...
    it('utilise profileDir personnalisé', () => {
        expect(resolveProfileDir('/custom/profiles', 'chrome')).toBe('/custom/profiles');
        expect(resolveProfileDir('/custom/profiles', 'canary')).toBe('/custom/profiles/chrome-profile-canary');
    });
});
// ---------------------------------------------------------------------------
// getForcedFlags
