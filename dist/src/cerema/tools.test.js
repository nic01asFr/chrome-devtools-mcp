...
function mockSessionManager(sessionId) {
    return {
        getSession: (id) => {
            if (id === sessionId) {
                return {
...
describe('registerL5Tools', () => {
    it('enregistre deux outils sur le McpServer', () => {
        ...
        expect(typeof shape.raison?.safeParse).toBe('function');
        expect(typeof shape.timeout_s?.safeParse).toBe('function');
    });
    it('raise_window est un outil sans paramètre', () => {
        ...
        const mockMcpServer = {
            ...
            registerTool(_name, _config, _cb) {
                // ignored
            },
        };
        registerL5Tools(mockMcpServer, mockSessionManager('x'), 'x');
